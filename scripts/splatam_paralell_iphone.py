...
def run_splatam(args):
    ...
    if 'jono' in seq_experiment.config['data']['gradslam_data_cfg']:
        ...
        run_name = f"splatam_{seq}/splatam_{seq}_{experiment_args['seed']}_{experiment_args['mov_init_by']}_{experiment_args['tracking_iters']}_{experiment_args['tracking_iters_init']}_{experiment_args['tracking_iters_cam']}_{experiment_args['l1_losses_embedding']}_{experiment_args['l1_losses_color']}_{experiment_args['bg_reg']}_{experiment_args['embeddings_lr']}_{experiment_args['red_lr']}_{experiment_args['red_lr_cam']}_{experiment_args['embedding_weight']}_{experiment_args['use_seg_for_nn']}_{experiment_args['weight_iso']}_{experiment_args['loss_weight_emb']}_{experiment_args['loss_weight_iso']}_{experiment_args['loss_weight_rigid']}_{experiment_args['loss_weight_rot']}_{experiment_args['loss_weight_depth_cam']}_{experiment_args['forward_propagate_camera']}_{experiment_args['stride']}_{experiment_args['l1_losses_scale']}_{experiment_args['kNN']}_{experiment_args['desired_image_height']}_{experiment_args['desired_image_width']}_{experiment_args['prune_gaussians']}_{experiment_args['use_depth_error_for_adding_gaussians']}_{experiment_args['add_only_bg']}_{experiment_args['loss_weight_depth_obj']}_{experiment_args['bg_loss']}_aniso_deb_l2_emb_{experiment_args['depth_type']}_{experiment_args['cam_type']}_segment_bug_rem_factor_{experiment_args['do_scale']}_bug_rem_hw_deb" #norm_embeddings
    # run_name = f"splatam_{seq}/splatam_{seq}_0_kNN_200_200_0_20_20_5_0.001_False_False_True_True_True_16_16_128_16_0.1_True_1_0_20_0.5_0.5_False_False_False_0.1_3_aniso_deb_l2_emb_aligned_depth_anything_colmap_refined_segment_bug_rem_factor_False_bug_rem_hw_deb"
    print(run_name)
    ...
    seq_experiment.config['data']['depth_type'] = experiment_args['depth_type']
    seq_experiment.config['data']['cam_type'] = experiment_args['cam_type']
    seq_experiment.config['data']['end'] = experiment_args['num_frames']
    seq_experiment.config['data']['do_scale'] = experiment_args['do_scale']
...
if __name__ == "__main__":
    ...
    experiment_args = dict(
        ...
        vis_all = True,
        vis_gt = True,
        tracking_iters = 200, #200,
        tracking_iters_init = 200, #200,
        tracking_iters_cam = 0,
        refine_iters = 0,
        mag_iso = True,
        init_jono = False,
        ...
        kNN=20,
        desired_image_height=1.0, # 240, # 120, #240, #480, 180, 360
        desired_image_width=1.0, # 455, # 227, # 455, #910, 320, 640
        instseg_obj=0.0,
        instseg_cam=0.0,
        smoothness=0,
        ...
        depth_type='aligned_depth_anything_colmap', # 'aligned_depth_anything_colmap', # 'lidar'
        cam_type='refined', # refined original
        do_scale=False,
        )
    
    iphone_seqs = ['haru-sit', 'sriracha-tree', 'paper-windmill', 'apple', 'backpack', 'block', 'creeper', 'mochi-high-five', 'pillow', 'spin', 'sriracha-tree', 'handwavy', 'paper-windmill', 'teddy'] #, 'teddy']

    configs_to_paralellize = list()
    for seq in iphone_seqs:
