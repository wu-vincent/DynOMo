...
import cv2
import flow_vis
from utils.eval_helpers import make_vid
...
class TrajEvaluator():
    ...
    def eval_traj(self):
        ...
        # get gt data
        data = get_gt_traj(self.config, in_torch=True, stereo=self.stereo, device=self.dev)
    ...
    def get_proj_and_params(self, primary_device):
        ...
        if self.cam is None:
            ...
            if not self.stereo and len(self.params['visibility'].shape) == 3:
    ...
    def vis_grid_trajs(
        ...
        data = get_gt_traj(self.config, in_torch=True, stereo=self.stereo)
        data['points'] = normalize_points(gs_traj_2D_for_vis, self.h, self.w).squeeze()    
