# PYTORCH VERSION OF FROM https://github.com/christophhagen/averaging-quaternions/tree/master
...
def averageQuaternions(Q):
    ...
    return torch.real(eigenVectors[:,0].A1)
...
def weightedAverageQuaternions(Q, w):
    # Number of quaternions to average
    M = Q.shape[0]
    A = torch.zeros(shape=(4,4))
    weightSum = 0

    for i in range(0,M):
        q = Q[i,:]
        A = w[i] * torch.outer(q,q) + A
        weightSum += w[i]
