...
def convert_data(cam_id=0, base_dir=''):
    ...
    cam_idxs = set([cam for k1, v1 in gt_2d_all.items() for k2, v2 in v1.items() for cam in v2.keys()])
    for cam_id in cam_idxs:
        all_2d_gt_converted_from_2d_cam = dict()
        for sequence in os.listdir(f'{base_dir}'):
            if sequence == 'annotations':
                continue
            if cam_id not in os.listdir(f'{base_dir}/{sequence}/ims'):
                continue
...
if __name__ == "__main__":
    convert_data(base_dir='data/panoptic_sport')